...
PUBLISH_RATE = 20 # make sure you increase it on a real car
class DBWNode(object):
    def __init__(self):
        ...
        min_speed = .1 # 1.0 * 0.447,
        self.controller = Controller(vehicle_mass=vehicle_mass, fuel_capacity=fuel_capacity,brake_deadband=brake_deadband,wheel_radius=wheel_radius,
                                     wheel_base=wheel_base, steer_ratio=steer_ratio, min_speed=min_speed,
    ...
    def loop(self):
        rate = rospy.Rate(PUBLISH_RATE) # 50Hz
        while not rospy.is_shutdown():
            ...
            if not None in (self.current_vel, self.linear_vel, self.angular_vel):
                ...
                if self.dbw_enabled:
                    # rospy.logwarn("dbw_node: throttle={}, brake={}, steering={} ".format(throttle, brake, steering))
